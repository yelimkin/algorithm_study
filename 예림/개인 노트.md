...
>   - StringBuilder와 StringBuffer는 변할 수 있는 객체이다. 문자열 연산 시, 기존 버퍼 크기를 늘리며 동작한다. StringBuilder와 StringBuffer는 제공하는 메소드가 서로 동일하지만 StringBuffer는 각 메서드별로 Synchronized Keyword가 존재하여, 멀티스레드 환경에서도 동기화를 지원하기 때문에 스레드에 안전한 프로그램이 필요할 때나, 개발 중인 시스템의 부분이 스레드에 안전한지 모를 경우 사용한다. StringBuilder는 동기화를 보장하지 않아 스레드에 안전한지 여부가 전혀 관계 없는 프로그램을 개발할 때 사용한다.
